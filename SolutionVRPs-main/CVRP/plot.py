...
def ge_test_data(device, size = 10, n_customer=20, seed = None) :
	print(n_customer)
	...
	for i in range(size):
		for j in range(n_customer):
			for k in range(n_customer):
				dist[i][j][k] = generate_data_real.get_dist(graph[i][j], graph[i][k]) #+ 0.1 * np.random.randn(1)
	demand = np.random.rand(size, n_customer)
	...
	demand = demand[0:size:, 1:n_customer]
	dist = dist[0:size]
...
'''if __name__ == '__main__':
	...
	pretrained = load_model(args.path, embed_dim = 64, n_customer = args.n_customer+1, n_encode_layers = 3)
	print(f'model loading time:{time()-t1}s')
	t2 = time()
	data = ge_test_data(device, 100, n_customer=args.n_customer+1)
	print(data[1].shape)
