...
def show_networkX_graph(figsize = (30,20)):
    ...
    # 把节点画出来
    nx.draw_networkx_nodes(G,point,node_color='b',node_size=30,alpha=0.8)
    # 把边画出来
    nx.draw_networkx_edges(G,point,width=1.0,alpha=0.8)
    #把边权重画出来
...
dis = np.zeros([1,42, 42])
for i in range(42):
    for j in range(42):
        dis[0][i][j] = cal_shortest_path(i, j)
np.save('dis_matrix.npy', dis)